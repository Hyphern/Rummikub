...
from config import LOG_TO_CSV, CSV_FILE_PATH, NUM_CPU_CORES
    ...
    for name in agent_names:
        if name == "Adaptive":
            save_file = os.path.join(
                _DATA_DIR, f"adaptive_weights_worker{worker_id}.json"
            )
...
def _run_single_game(args):
    ...
    if winner_idx is not None and 0 <= winner_idx < len(game_agents):
        winner_name = game_agents[winner_idx].name
    else:
        winner_idx = -1
        winner_name = "Draw"
...
class MatchupTracker:
    ...
    def __init__(self, csv_path: str = os.path.join(_DATA_DIR, CSV_FILE_PATH)):
    ...
    def print_matrix(self):
        ...
        print()
        print(
            "(Row = agent, Column = opponent, Value = row agent's win% (games played))"
        )
        print()
...
class ProgressTracker:
    ...
    def __init__(
        self, total_games: int, update_interval: float = 1.0, num_workers: int = 1
    ):
    ...
    def update(self, game_result: Dict, worker_id: Optional[int] = None):
        ...
        if (
            ...
            or self.completed_games == self.total_games
        ):
    ...
    def _display_progress(self):
        ...
        print(
            f"Games: {self.completed_games}/{self.total_games} | "
        ...
        if self.num_workers > 1:
            active = (
                ", ".join(f"W{i}" for i in sorted(self.active_workers))
        ...
        if self.recent_results:
            ...
            for result in self.recent_results[-3:]:
                print(
                    f"  {result['agents'][0]} vs {result['agents'][1]}: {result['winner_name']}"
                )
            print()
        ...
        print(
            f"{'Rank':<6} {'Agent':<20} {'Wins':<8} {'Games':<8} {'Win %':<10} {'Streak':<8}"
        )
        print("-" * 80)
        ...
        for rank, (agent, wins, games, win_rate, streak) in enumerate(standings, 1):
            ...
            print(
                f"{rank:<6} {agent:<20} {wins:<8} {games:<8} {win_rate:>6.1f}%   {streak_str:<8}"
            )
...
class Tournament:
    ...
    def __init__(
        self,
        agents: List[RummikubAgent],
        num_players: int = 2,
        matchup_csv: Optional[str] = os.path.join(_DATA_DIR, CSV_FILE_PATH),
    ):
    ...
    def run_game(
        ...
        if len(agent_indices) != self.num_players:
            raise ValueError(
                f"Need {self.num_players} agents, got {len(agent_indices)}"
            )
        ...
        while not done and step < self.env.max_steps:
            ...
            if verbose and info.get("action_success"):
                print(
                    f"Step {step}: {agent.name} took {info['action_taken']} "
                    f"(reward: {reward:.2f})"
                )
        ...
        if step >= self.env.max_steps:
            ...
            for pid in range(self.num_players):
                print(
                    f"  Player {pid + 1} initial meld: {self.env.game_state.has_initial_meld[pid]}"
                )
        ...
        if self.matchup_tracker:
            hand_sizes = [
                len(self.env.game_state.player_hands[i])
            ...
            for j in range(num_agents):
                ...
                for game_num in range(games_per_matchup):
                    ...
                    if verbose and not show_progress and game_num == 0:
                        print(
                            f"{self.agents[i].name} vs {self.agents[j].name}: "
    ...
    def run_random_matchups(
        ...
        for game_num in range(num_games):
            ...
            if verbose and not show_progress and game_num % 10 == 0:
                print(
                    f"Game {game_num + 1}/{num_games}: Winner = {result['winner_name']}"
                )
    ...
    def run_random_matchups_parallel(
        self,
        num_games: int = 100,
        ...
        if num_workers is None:
            num_workers = max(1, NUM_CPU_CORES)
        ...
        tracker = ProgressTracker(
            num_games, update_interval=progress_interval, num_workers=num_workers
        )
        tracker.start()

        results = []
        worker_stats = {
            i: {"wins": 0, "games": 0, "active": False} for i in range(num_workers)
        }

        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            futures = {
                executor.submit(_run_single_game, args): args for args in game_args
            }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if "Adaptive" in result["agents"]:
                        ...
                        if (
                            winner_idx >= 0
                            and result["agents"][winner_idx] == "Adaptive"
                        ):
        ...
        if "Adaptive" in agent_names:
            best_worker = max(
                worker_stats.items(), key=lambda x: x[1]["wins"] / max(1, x[1]["games"])
            )
            ...
            best_file = os.path.join(
                _DATA_DIR, f"adaptive_weights_worker{best_worker_id}.json"
            )
    ...
    def print_results(self):
        ...
        sorted_agents = sorted(
            results["agent_stats"].items(), key=lambda x: x[1]["win_rate"], reverse=True
        )

        for agent_name, stats in sorted_agents:
            print(
                f"{agent_name:<20} {stats['games']:<8} {stats['wins']:<8} "
...
if __name__ == "__main__":
    ...
    tournament.run_random_matchups_parallel(
        num_games=10, verbose=True, progress_interval=2.0
    )
