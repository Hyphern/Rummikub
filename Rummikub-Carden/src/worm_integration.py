...
sys.path.insert(0, os.path.join(_SCRIPT_DIR, "..", "testing_files", "worm"))
from worm import (
    find_explicit_moves,
    get_all_runs,
    get_all_groups,
...
def combine_boards(
    hand_board: List[List[int]], table_board: List[List[int]]
) -> List[List[int]]:
...
def find_combined_moves(
    hand: List[Tile], table_melds: List[Meld]
) -> Dict[str, List[Tuple]]:
    ...
    """
    result = {"hand_only": [], "add_to_meld": [], "combined": []}
...
def score_move_for_hoarding(
    move: Tuple, hand: List[Tile], table_melds: List[Meld]
) -> float:
...
def get_best_move_for_strategy(
    hand: List[Tile], table_melds: List[Meld], strategy: str = "balanced"
) -> Optional[Tuple]:
    ...
    if strategy == "hoarding":
        scored_moves = [
            (move, score_move_for_hoarding(move, hand, table_melds)) for move in moves
        ]
    elif strategy == "aggressive":
        scored_moves = [
            (move, score_move_for_aggressive(move, hand, table_melds)) for move in moves
        ]
    else:  # balanced
        scored_moves = [
            (move, score_move_for_balanced(move, hand, table_melds)) for move in moves
        ]
...
def can_go_out_in_one_move(hand: List[Tile], table_melds: List[Meld]) -> bool:
...
def count_playable_tiles(hand: List[Tile], table_melds: List[Meld]) -> int:
