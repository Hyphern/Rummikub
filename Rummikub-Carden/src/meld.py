...
class Meld:
    ...
    def is_valid_run(cls, tiles: List[Tile]) -> bool:
        ...
        regular_sorted = sorted(
            regular, key=lambda t: t.number if t.number is not None else 0
        )
        ...
        for i in range(len(regular_sorted) - 1):
            num1 = (
                regular_sorted[i].number if regular_sorted[i].number is not None else 0
            )
            num2 = (
                regular_sorted[i + 1].number
                if regular_sorted[i + 1].number is not None
                else 0
            )
            gap = num2 - num1 - 1
            if gap < 0:
    ...
    def get_valid_additions(
        cls, meld_tiles: List[Tile], hand: List[Tile]
    ) -> List[Tile]:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        self_sorted = sorted(
            self.tiles, key=lambda t: (t.color or "", t.number or 0, t.is_joker)
        )
        other_sorted = sorted(
            other.tiles, key=lambda t: (t.color or "", t.number or 0, t.is_joker)
        )
        return self_sorted == other_sorted

    def get_joker_replacements(self) -> List[Tile]:
        """Get the possible tiles that each joker in this meld could represent.

        For runs: returns the tile that fills the gap the joker is filling.
        For groups: returns the tile of the missing color.

        Returns:
            List of tiles that could replace each joker (in order of jokers in meld)
        """
        jokers = [t for t in self.tiles if t.is_joker]
        if not jokers:
            return []
        ...
        if self.is_valid_run(self.tiles):
            # For runs, figure out which numbers are missing
            regular = [t for t in self.tiles if not t.is_joker]
            if not regular:
                return []

            color = regular[0].color
            regular_sorted = sorted(regular, key=lambda t: t.number)

            # Find gaps and map jokers to them
            numbers = [t.number for t in regular_sorted]
            all_numbers = set(range(1, 14))
            present = set(numbers)
            missing = sorted(all_numbers - present)

            # Assign jokers to missing numbers
            for i, joker in enumerate(jokers):
                if i < len(missing):
                    replacements.append(Tile(color=color, number=missing[i]))
                else:
                    # Extra jokers could extend the run
                    if numbers:
                        last_num = max(numbers)
                        if last_num < 13:
                            replacements.append(Tile(color=color, number=last_num + 1))

        elif self.is_valid_group(self.tiles):
            # For groups, figure out which colors are missing
            regular = [t for t in self.tiles if not t.is_joker]
            if not regular:
                return []

            number = regular[0].number
            present_colors = set(t.color for t in regular)
            all_colors = set(Tile.VALID_COLORS)
            missing_colors = sorted(
                all_colors - present_colors,
                key=lambda c: ["red", "blue", "black", "orange"].index(c),
            )

            # Assign jokers to missing colors
            for i, joker in enumerate(jokers):
                if i < len(missing_colors):
                    replacements.append(Tile(color=missing_colors[i], number=number))
                else:
                    # Extra jokers - just repeat one of the missing
                    if missing_colors:
                        replacements.append(
                            Tile(color=missing_colors[0], number=number)
                        )
...
def find_best_initial_meld(tiles: List[Tile]) -> Optional[List[Tile]]:
    ...
    best_meld = None
    best_value = INITIAL_MELD_MIN_VALUE  # Minimum threshold
