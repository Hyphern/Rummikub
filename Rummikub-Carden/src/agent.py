...
class RummikubAgent(ABC):
    ...
    @abstractmethod
    def select_action(
        self, observation: Dict[str, np.ndarray], valid_actions: np.ndarray
    ) -> int:
...
class SmartWormAgent(RummikubAgent):
    ...
    # Action encoding constants (must match ml_environment.py)
    ACTION_DRAW = 0
    ACTION_END_TURN = 1
    ACTION_DECLARE_OUT = 2
    ACTION_PLAY_NEW_MELD_BASE = 3
    MAX_HAND_SIZE = 30  # Must match RummikubMLEnv.max_hand_size
    MAX_MELDS = 40  # Must match RummikubMLEnv.max_melds
    ACTION_ADD_TO_MELD_BASE = 3 + 30  # ACTION_PLAY_NEW_MELD_BASE + MAX_HAND_SIZE
    ACTION_REARRANGE_BASE = (
        3 + 30 + (30 * 40)
    )  # 1233 — must match RummikubMLEnv.REARRANGE_BASE
    ...
    def select_action(
        ...
        # Always declare out if possible
        if self.ACTION_DECLARE_OUT in valid_indices:
            return self.ACTION_DECLARE_OUT
        ...
        if not self.worm_available:
            ...
            meld_actions = [
                ...
                if self.ACTION_PLAY_NEW_MELD_BASE <= a < self.ACTION_ADD_TO_MELD_BASE
            ]
            if meld_actions:
                return random.choice(meld_actions)
            # End turn if we played melds, otherwise draw
            if self.ACTION_END_TURN in valid_indices:
        ...
        if action is not None:
            return action
        # No play action found — end turn if we played melds, else draw
        if self.ACTION_END_TURN in valid_indices:
            return self.ACTION_END_TURN
        return 0 if 0 in valid_indices else random.choice(valid_indices)
    ...
    def _find_best_action(
        ...
        if self.DEBUG:
            print(
                f"  [SmartWorm] hand={len(hand)} tiles, table={len(table)} melds, has_initial={has_initial}",
                flush=True,
            )
        ...
        # ── Step 1: Initial meld — play melds toward threshold (accumulates) ──
        if not has_initial:
            if self.DEBUG:
                print(
                    f"  [SmartWorm] Initial meld search: {len(cached_melds)} valid melds",
                    flush=True,
                )
            # Sort by value descending so we play the highest-value meld first.
            # With accumulation rules, any valid meld can contribute toward the
            # initial meld threshold. The game state tracks the running total.
            ...
            for meld_tiles in cached_melds:
                ...
                if action is not None:
                    if self.DEBUG:
                        print(
                            f"  [SmartWorm] Playing initial meld worth {Meld.calculate_value(meld_tiles)}",
                            flush=True,
                        )
                    return action
            # Can't make any valid meld — must draw (rollback happens automatically)
            if 0 in valid_indices:
                if self.DEBUG:
                    print(f"  [SmartWorm] Drawing (no initial meld)", flush=True)
                return 0
            return None
        ...
        combined_board = [
            [table_board[r][c] + hand_board[r][c] for c in range(13)] for r in range(4)
        ]
        ...
        if self.DEBUG:
            print(
                f"  [SmartWorm] Found {len(all_definite)} definite moves on combined board",
                flush=True,
            )
        ...
        if tiles_to_play:
            if self.DEBUG:
                print(
                    f"  [SmartWorm] Definite moves consume {len(tiles_to_play)} hand tiles",
                    flush=True,
                )
            for tile_pos in tiles_to_play:
                action = self._play_tile_from_hand(
                    tile_pos, hand, table, valid_indices, cached_melds
                )
                if action is not None:
                    return action
        ...
        if cached_melds:
            ...
            for meld_tiles in sorted_melds:
                ...
                if action is not None:
                    if self.DEBUG:
                        print(
                            f"  [SmartWorm] Playing meld from hand ({len(meld_tiles)} tiles)",
                            flush=True,
                        )
                    return action
        ...
        if add_action is not None:
            if self.DEBUG:
                print(f"  [SmartWorm] Adding tile to existing meld", flush=True)
            return add_action
        ...
        if meld_actions:
            if self.DEBUG:
                print(
                    f"  [SmartWorm] Fallback: found {len(meld_actions)} valid PLAY_NEW_MELD actions",
                    flush=True,
                )
        ...
        add_actions = [
            a
            for a in valid_indices
            if a >= self.ACTION_ADD_TO_MELD_BASE and a < self.ACTION_REARRANGE_BASE
        ]
        if add_actions:
            if self.DEBUG:
                print(
                    f"  [SmartWorm] Fallback: found {len(add_actions)} valid ADD_TO_MELD actions",
                    flush=True,
                )
            return add_actions[0]

        # ── Step 3b: Try rearrangement ──
        # If no standard play actions remain, try rearranging the table to
        # place stuck hand tiles.
        rearrange_actions = [
            a for a in valid_indices if a >= self.ACTION_REARRANGE_BASE
        ]
        if rearrange_actions:
            if self.DEBUG:
                print(
                    f"  [SmartWorm] Trying rearrangement ({len(rearrange_actions)} options)",
                    flush=True,
                )
            return rearrange_actions[0]

        # ── Step 4: End turn if we played melds, else draw ──
        if self.ACTION_END_TURN in valid_indices:
            if self.DEBUG:
                print(f"  [SmartWorm] Ending turn", flush=True)
            return self.ACTION_END_TURN

        if 0 in valid_indices:
            if self.DEBUG:
                print(f"  [SmartWorm] Drawing (last resort)", flush=True)
            return 0

        return None

    def _play_tile_from_hand(
        self,
        tile_pos: tuple,
        hand: List[Any],
        table: List[Any],
        valid_indices: np.ndarray,
        cached_melds: Optional[List] = None,
    ) -> Optional[int]:
            ...
            if (
                ...
                for meld_idx, meld in enumerate(table):
                    if Meld.can_add_tile(meld.tiles, tile):
                        action_idx = (
                            self.ACTION_ADD_TO_MELD_BASE
                            + tile_idx * self.MAX_MELDS
                            + meld_idx
                        )
                ...
                melds = (
                    cached_melds
                    if cached_melds is not None
                    else find_all_valid_melds(hand)
                )
    ...
    def _find_add_to_meld_action(
        self, hand: List[Any], table: List[Any], valid_indices: np.ndarray
    ) -> Optional[int]:
    ...
    def _meld_to_action(
        self, meld_tiles: List[Any], hand: List[Any], valid_indices: np.ndarray
    ) -> Optional[int]:
        """Convert a meld (list of tiles) to an action index.

        Action encoding: action = ACTION_PLAY_NEW_MELD_BASE + tile_hand_index.
        ...
        for idx in tile_indices:
            action_idx = self.ACTION_PLAY_NEW_MELD_BASE + idx
            if action_idx in valid_indices:
                return action_idx
...
class WeightedWormAgent(SmartWormAgent):
    ...
    def _find_best_action(
        ...
        if self.DEBUG:
            print(
                f"  [{self.name}] hand={len(hand)}, table={len(table)}, "
                f"agg={self.aggressiveness}, meld_pref={self.meld_size_preference}, "
                f"draw_pref={self.draw_preference}",
                flush=True,
            )

        cached_melds = find_all_valid_melds(hand)

        # ── Step 1: Initial meld (accumulation — play any valid meld toward threshold) ──
        if not has_initial:
            if not cached_melds:
            ...
            if self.meld_size_preference > 0.5:
                ...
                cached_melds.sort(key=lambda m: Meld.calculate_value(m))
        ...
        if should_draw_instead:
            if self.DEBUG:
                print(
                    f"  [{self.name}] Drawing (hand={hand_size} < threshold={play_threshold}, "
                    f"draw_pref={self.draw_preference})",
                    flush=True,
                )
            return 0
        ...
        if hand_size >= play_threshold:
            ...
            add_actions = [
                a
                for a in valid_indices
                if self.ACTION_ADD_TO_MELD_BASE <= a < self.ACTION_REARRANGE_BASE
            ]
            if add_actions:
                return add_actions[0]

            # Try rearrangement before giving up
            rearrange_actions = [
                a for a in valid_indices if a >= self.ACTION_REARRANGE_BASE
            ]
            if rearrange_actions:
...
class AdaptiveWormAgent(WeightedWormAgent):
    ...
    def _update_weights(self, won: bool):
        ...
        if won:
            ...
            for key in current:
                blend = (
                    current[key] * (1 - lr * self.win_momentum)
